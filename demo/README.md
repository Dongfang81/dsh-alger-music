...
`moony-singer-demo.mp4` —— 63 秒功能介绍演示（真实驱动插件与 App）：
...
- 直接播放 `moony-singer-demo.mp4`（H.264，微信/邮件可直接发）。
...
ffmpeg -y -ss 13 -i "$V" -vf "crop=440:580:860:160,scale=880:1160:flags=lanczos,fade=t=in:st=0:d=0.6,fade=t=out:st=61.5:d=0.8" \
  -c:v libx264 -crf 20 -pix_fmt yuv420p -movflags +faststart moony-singer-demo.mp4
```
