			...
			if (player) {
				try {
					...
					out.favorite = Array.isArray(player.favoriteList)
						...
						: false;
					out.playMode = typeof player.playMode === 'number' ? player.playMode : null;
				} catch {
					out.favorite = false;
					out.playMode = null;
				}
...
/**
 * 构造“切换播放模式”脚本：调用 player.togglePlayMode() 循环切换
 * 0=列表循环 / 1=单曲循环 / 2=随机，返回切换后的模式。
 */
export function buildTogglePlayModeScript() {
	return `(async () => {
		const out = { ok: false };
		try {
			const app = document.querySelector('#app');
			...
			const pinia = g && g.$pinia;
			const player = pinia && pinia._s && pinia._s.get('player');
			if (!player || typeof player.togglePlayMode !== 'function') {
				out.error = '未找到播放模式切换（App 版本不兼容？）';
				return out;
			}
